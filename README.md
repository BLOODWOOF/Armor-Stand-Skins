...
Player Armor Stands is not required.
...
[Armor Poser](https://modrinth.com/mod/armor-poser) is optional. If it is loaded, its armor stand screen gets a seventh toggle under the usual six. That toggle turns head skins off for that stand only and saves on the entity (`HeadSkinDisabled`). Put this jar on the server as well if you want the flag to persist and stay in sync for other players.
...
The output jar is `build/libs/head-skins-<version>.jar`. Compile-only copies of Armor Poser and 3D Skin Layers belong in `libs/` and are not shipped in the jar.
